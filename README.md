...
**用户建模、热点洞察、内容架构、平台运营、商业化评估、知识管理 —— 你负责灵感，它们负责落地。**
...
> 早上刷到一条行业动态，你隐约觉得可以做成内容 —— 但不确定值不值得投入。输入一句话灵感，4 个 Agent 在 30 秒内完成用户验证、热点匹配、脚本创作和商业化评估，输出一份可直接执行的内容方案。

### 4 节点 Agent 接力

| 节点 | Agent | 产出 |
|------|-------|------|
| 01 用户建模 | `user-research-architect` | 锚定目标用户画像与 JTBD |
| 02 热点匹配 | `trend-intelligence-scout` | 基于用户画像的热点校准与选题验证 |
| 03 脚本创作 | `video-content-architect` | 多平台适配的内容产出 |
| 04 商业化评估 | `monetization-strategist` | 变现路径与定价策略 |

**Dify Workflow 链接**：`https://udify.app/workflow/eg0F0sOLw6P5Rqfn`
...
|---|-------|-----------|---------|------|
| 1 | `user-research-architect` | 👤 用户建模助手 | 每次发布前，验证内容是否精准命中目标用户的 Job-to-be-Done | ✅ 已上线 |
| 2 | `trend-intelligence-scout` | 🔥 热点侦查助手 | 每天获取 3-5 个经过验证的热点选题，附带热度评分与关联知识标注 | ✅ 已上线 |
| 3 | `video-content-architect` | 🎬 内容架构助手 | 输入一个话题，输出小红书/B站/YouTube 三平台的完整脚本 | ✅ 已上线 |
...
```html
<!-- 用户建模助手 -->
<iframe src="https://udify.app/chat/le5nzqDHAgzeKS3A"></iframe>

<!-- 热点侦查助手 -->
<iframe src="https://udify.app/chat/bLSjoBFFCt0ZcHb7"></iframe>

<!-- 内容架构助手 -->
<iframe src="https://udify.app/chat/eUtE2pqQLOECuH14"></iframe>
```
...
**工作流**：灵感输入 → 用户建模 → 热点匹配 → 脚本创作 → 商业化评估 → 知识沉淀
